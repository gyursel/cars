...
function applyBranding(heroUrl, logoUrl) {
  ...
  if (!img) {
    ...
    Object.assign(img.style, { position:'absolute', left:'50%', top:'50%', transform:'translate(-50%, -50%)', zIndex:'7', width:'min(70vw, 520px)', maxHeight:'42%', objectFit:'contain', pointerEvents:'none', filter:'drop-shadow(0 4px 14px rgba(0,0,0,.7))' });
...
export default function LogoManager() {
  ...
  async function upload(file, kind) {
    ...
    try {
      ...
      await setDoc(brandingRef(), { [field]:url }, { merge:true });
      if (kind === 'hero') setHeroUrl(url); else setLogoUrl(url);
      setMessage(kind === 'hero' ? 'Главният банер е записан ✓' : 'Логото е записано ✓');
    } catch (err) { setMessage(`Грешка: ${err.message || 'неуспешно качване'}`); }
    finally { setUploading(''); if (logoInputRef.current) logoInputRef.current.value=''; if (heroInputRef.current) heroInputRef.current.value=''; }
  }
  ...
  return <div style={{position:'fixed',right:14,bottom:14,zIndex:9999,width:'min(350px, calc(100vw - 28px))',background:'rgba(18,21,26,.98)',border:'1px solid rgba(201,162,39,.55)',borderRadius:12,padding:12,boxShadow:'0 16px 40px rgba(0,0,0,.35)',color:'#fff',maxHeight:'70vh',overflowY:'auto'}}>
    <div style={{color:'#E8B830',fontSize:11,fontWeight:800,letterSpacing:'.12em',marginBottom:6}}>ГЛАВЕН БАНЕР</div>
    <div style={{color:'#AEB4BC',fontSize:11,lineHeight:1.45,marginBottom:9}}>Качи цялата снимка за горния банер. Показва се 1:1 без изрязване на телефон и компютър.</div>
