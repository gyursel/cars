...
function extractDetail(html, sourceUrl) {
  ...
  const productionDate = findValue(text, 'Дата на производство', labels.slice(1));
  ...
  const displacement = findValue(text, 'Кубатура [куб.см]', labels.slice(5));
  const gearbox = findValue(text, 'Скоростна кутия', labels.slice(6));
  const category = findValue(text, 'Категория', labels.slice(7));
  const mileage = findValue(text, 'Пробег [км]', labels.slice(8));
