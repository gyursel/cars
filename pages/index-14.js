...
export default function Home() {
  ...
  return (
    <>
      ...
      {screen === 'lot' && lot && lot.demoExpired && (
        <div style={{background:'#080a0c',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{position:'sticky',top:0,left:0,right:0,zIndex:50,background:'#C9302C',color:'white',textAlign:'center',padding:'16px 20px',fontSize:15,fontWeight:700,letterSpacing:'.02em'}}>
            ⚠ Пробният период е приключил — това е демо версия на приложението.
          </div>
          <button onClick={()=>{setPassword('');setLoginError(false);setScreen('login')}} style={{position:'fixed',right:12,bottom:12,zIndex:8,background:'#0b0d0ecc',border:'1px solid #8b6d32',color:'#d8ad52',padding:'6px 9px',borderRadius:5}}>⚙</button>
        </div>
      )}
